...
class GithubRepositoriesCacheImpl @Inject constructor(private val db: Database) :
    IGithubRepositoriesCache {

    override fun getUserRepos(user: GithubUser): Single<List<GitHubRepo>> =
        Single.fromCallable {
            val roomUsers = user.login?.let { db.userDao.findByLogin(it) }
                ?: throw RuntimeException("No user in cache")
